...
def moveTP(trade, new_tp: float) -> bool:
    ...
    if float(take_profit_trade) != new_tp:
        ...
        if result_modify_request.retcode != Mt5.TRADE_RETCODE_DONE:
            ...
            print(colored(f"\n{'-' * 100}\n", "blue"))
            print(colored(f"successfully moved TP", "green"))
...
def moveSL(trade, new_sl: float) -> bool:
    ...
    if float(stop_loss_trade) != new_sl:
        ...
        if result_modify_request.retcode != Mt5.TRADE_RETCODE_DONE:
            ...
            print(colored(f"\n{'-' * 100}\n", "blue"))
            print(colored(f"successfully moved SL", "green"))
...
def getIntervalInTimeframe(TF_name: str):
    if TF_name == "M1":
        ...
        time_interval = timedelta(days=0, hours=0, minutes=5)
    elif TF_name == "M15":
        time_interval = timedelta(days=0, hours=0, minutes=15)
    elif TF_name == "M30":
        time_interval = timedelta(days=0, hours=0, minutes=30)
    elif TF_name == "H1":
        time_interval = timedelta(days=0, hours=1, minutes=0)
    elif TF_name == "H4":
        time_interval = timedelta(days=0, hours=4, minutes=0)
    elif TF_name == "D1":
        time_interval = timedelta(days=1, hours=0, minutes=0)
    else:
        raise "Unrecognized TF name"
