...
class Trade:
    ...
    def __init__(
        ...
        self.price = trade_object.price
        self.tp = trade_object.tp
        ...
        if self.tp is not None:
            self.request_open["tp"] = self.tp
        if self.sl is not None:
            self.request_open["sl"] = self.sl
    ...
    def open_position(
        ...
        this market order can be :
            ...
            - sell stop
        """
        account_currency = my_account.account_currency
        ...
        if self.result_open_request.retcode != Mt5.TRADE_RETCODE_DONE:
            logMessageDate(LOGGER_GLOBAL.error, f"Failed to send order :(, retcode: {self.result_open_request.retcode}")
            logMessageDate(LOGGER_GLOBAL.error, self.result_open_request)
            return False, self.result_open_request
        else:
            self.ticket_order = self.result_open_request.order
            logMessageDate(LOGGER_GLOBAL.info, "Order successfully placed!")
    ...
    def close_position(self, my_account: Account) -> bool:
        ...
        if self.ticket_order in my_account.trade_pending:
            ...
            logMessageDate(LOGGER_GLOBAL.error, "This order is no longer on pending or on going so cannot proceed to close")
            return False
        ...
        if result_close_request.retcode != Mt5.TRADE_RETCODE_DONE:
            logMessageDate(LOGGER_GLOBAL.error, f"Failed to close order :( \n {result_close_request}")
            return False
        else:
            logMessageDate(LOGGER_GLOBAL.info, "Order successfully closed!")
